...
#print "YOUR JOUNEY IS FROM\t" ,source, "to" ,destination, "where:\n"
...
for i in range(0,len(master)):
	...
	print "Approximate travel time from ",T1," to ",T2," is ",Time_in_minutes ,"minutes"
	for k in Dist:
		D = k
		for p in Sig_Time:
			Time_Cycle = p
	...
	print "Cycles that will be elapsed by the time destination is reached: "
	print Cycles_Elapsed
	Cycle_Fraction = Cycles_Elapsed - int(Cycles_Elapsed)
	print "Fraction of ongoing cycle: "
	...
	if x > (0.6 * master[i]["Signal Time"]):
		print"if ongoing cycle falls under red light cycle then taken in consideration for average speed:"
		Time_Fraction = master[i]["Signal Time"] - x #Time_Fraction is to be added to the time by which distance is to be divided
		print Time_Fraction
		print "The Time_fraction added with approx. time travel is total time for average speed" 
		Average_Speed = D/(Time_Fraction + Time_Approx + 120)
	else:
		Average_Speed = D/(Time_Approx +120)
	AVERAGE_SPEED = (Average_Speed * 3600)+25	
	print "\nAverage speed from ",T1," to ",T2," is ",AVERAGE_SPEED, "km/hr\n"
